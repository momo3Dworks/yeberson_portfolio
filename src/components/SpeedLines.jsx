...
export default function SpeedLines({ shipRef, speedFactor }) {
  const MAX_LINES  = 40;   // instanced pool size
  ...
  const material = useMemo(
    () =>
      new THREE.MeshBasicMaterial({
        color: 0xff00ff, // magenta to match engine burst color
        transparent: true,
        opacity: 0.85,
  ...
  return <instancedMesh ref={meshRef} args={[geometry, material, MAX_LINES]} />;
}
