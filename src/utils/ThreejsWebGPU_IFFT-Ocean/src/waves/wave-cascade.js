...
import { textureStore, instanceIndex, uniform, uint, storage, workgroupId, localId, time } from "three/tsl";
...
class WaveCascade extends entity.Component {
	...
	Init(params) {
		...
		const maxAnisotropy = (this.params_.renderer.capabilities && this.params_.renderer.capabilities.getMaxAnisotropy)
			? this.params_.renderer.capabilities.getMaxAnisotropy()
			: 16;
		this.displacement.anisotropy = maxAnisotropy;
		this.derivative.anisotropy = maxAnisotropy;
		this.jacobian.anisotropy = maxAnisotropy;
		...
		const u_oceanSpeed = uniform(2.0);
